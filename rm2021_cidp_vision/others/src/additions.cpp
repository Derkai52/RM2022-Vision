...
cv::VideoWriter initVideoWriter(const std::string &filename_prefix) {  // 视频文件写入
...
auto video_writer = initVideoWriter(PROJECT_DIR"/video/");  // 相机保存路径,已弃用

void isFileExist(){ // 判断文件是否存在,不存在则创建
	const char* fname = PROJECT_DIR"/video";
...
void saveVideos(const cv::Mat &gimbal_src) {   // 保存相机录制视频

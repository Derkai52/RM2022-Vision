...
extern MV_CC_DEVICE_INFO *pDeviceInfo; // Warning: 并不是一个可读性高的写法。可能引起混乱和冲突。
...
void debugImg() {
    // 滑动条初始化
    namedWindow("box"); // bug ,关闭一次窗口后，后面的窗口没有滑动条。应当设置窗口不可被手动关闭
    ...
    createTrackbar(TrackbarName, "box", &exposureTime_slider, exposureTime_slider_max, exposureTrackbar); // 曝光时间
    exposureTrackbar(exposureTime_slider, 0);

    sprintf(TrackbarName, "Gain(Max:%.4lf)", gain_slider_max);
    createTrackbar(TrackbarName, "box", &gain_slider, gain_slider_max, gainTrackbar); // gain增益值
    gainTrackbar(gain_slider, 0); // Warning：建议按实际情况控制步长

    sprintf(TrackbarName, "Gamma (Max:%.4lf dB)", gammas_slider_max / 1000);
    createTrackbar(TrackbarName, "box", &gammas_slider, gammas_slider_max, gammasTrackbar);
    gammasTrackbar(gammas_slider, 0); // Warning：步长控制在小数点后1位

    // 白平衡
    sprintf(TrackbarName, "BalanceRatioRed (Max:%d)", balanceRatioRed_slider_max);
    createTrackbar(TrackbarName, "box", &balanceRatioRed_slider, balanceRatioRed_slider_max, balanceRatioRedTrackbar);
    ...
    createTrackbar(TrackbarName, "box", &balanceRatioGreen_slider, balanceRatioGreen_slider_max,
                   balanceRatioGreenTrackbar);
    ...
    createTrackbar(TrackbarName, "box", &balanceRatioBlue_slider, balanceRatioBlue_slider_max,
                   balanceRatioBlueTrackbar);

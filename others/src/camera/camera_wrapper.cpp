...
bool CameraWrapper::init() {
    ...
    cout << "camera_enumerate_device_status"<<camera_enumerate_device_status<< endl;
    if (camera_enumerate_device_status != CAMERA_STATUS_SUCCESS) {
        ...
        cout << "camera_cnts"<<camera_cnts<<endl;
    }
    if (camera_cnts == 0) {  // 开启后因摄像头松动离线执行此行
        LOGE("No!!!!!!!!!!!!!!!!1 camera device detected!");
        return false;

...
void CamInfoSet(IN void *handle) {
    ...
    // 设置曝光模式
    nRet = MV_CC_SetExposureAutoMode(handle, 2); // 【0：关闭自动曝光 1：仅初次自动曝光 2：连续自动曝光】
    if (MV_OK != nRet) {
...
//        CamInfoSet(handle); // 可以通过【相机调试模式】或 MVS设置图像属性信息，也可通过此函数手动设置

...
bool run_by_frame = false; // 直接通过视频流启动程序
...
std::map<std::string, std::pair<std::string, void(*)(void)>> options = {
    ...
    {"--match-red", {
        "Competition use:Enemy target RED and save video.", [](){
//            save_video = true;
            show_armor_box = true;
            LOGM("Enable save video!");
            color = false;
            LOGM("Enemy target: RED");
        }
    }}, 
    {"--match-blue", {
        "Competition use:Enemy target BLUE and save video.", [](){
//            save_video = true;
            LOGM("Enable save video!");
            color = true;
            LOGM("Enemy target: BLUE");
        }
    }},
    {"--debug-red", {

...
static uint8_t get_blob_color(const cv::Mat &src, const cv::RotatedRect &blobPos) {
    ...
    for (int row = 0; row < roi.rows; row++) {                 // TODO: 微小矩形易因统计量导致判断失能(噪声像素占比大)
...
bool ArmorFinder::findLightBlobs(const cv::Mat &src, LightBlobs &light_blobs)
{
    ...
    cv::threshold(color_channel, src_bin_dim, 180, 255, CV_THRESH_BINARY); // 二值化对应通道
    ...
    for (int l = 0; l != light_blobs_light.size(); l++)
    {
        ...
        {
            if (isSameBlob(light_blobs_light[l], light_blobs_dim[d]))  // 判断是否为同一灯条
            {
    ...
    sort(light_to_remove.begin(), light_to_remove.end(), [](int a, int b) { return a > b; });  // 检索方式为按面积大小从大到小排序
    sort(dim_to_remove.begin(), dim_to_remove.end(), [](int a, int b) { return a > b; });      // 检索方式为按面积大小从大到小排序

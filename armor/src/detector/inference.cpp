...
std::map<int, string> id2name = {                               //装甲板id到名称的map
        {-1, "NO"}, {0, "B8"},{ 1, "B1"},{ 2, "B2"},{ 3, "B3"},{ 4, "B4"},{ 5, "B5"},{ 6, "B6"},{ 7, "B7"},
        {8, "R8"}, { 9, "R1"},{ 10, "R2"},{11, "R3"},{12, "R4"},{13, "R5"},{14, "R6"},{15, "R7"}
};

std::map<string, int> name2id = {                               //装甲板名称到id的map
        {"NO", -1},{"B8",  0},{"B1", 1},{"B2",  2},{"B3",  3},{"B4",  4},{"B5",  5},{"B6",  6}, {"B7",  7},
        {"R8", 8}, {"R1",  9},{"R2", 10},{"R3", 11},{"R4", 12},{"R5", 13},{"R6", 14},{"R7", 15}
};

// TODO: 优先级和内容还需要更改(暂未启用)
...
ArmorFinder::ArmorFinder(uint8_t &color, Serial &u):
        serial(u),
        enemy_color(color),
        state(STANDBY_STATE){
}
...
bool ArmorFinder::initModel(string path)
{
...
bool ArmorFinder::detect(Mat &src, std::vector<ArmorObject>& objects)
{
    if (src.empty())
    {
...
void ArmorFinder::run(cv::Mat &src) {
    ...
    switch (SEARCHING_STATE)  {  // TODO: 状态机切换，目前仅使用备用模式

...
//    cout << box.lengthDistanceRatio() << endl;
    ...
    if(box.getOrientation() == ArmorBox::FRONT){  // TODO: 判断朝向误差过大
        rectangle(image2show, box.rect, Scalar(0, 255, 0), 3); // 使用粗框表示朝向正面的装甲板
    }else{
        rectangle(image2show, box.rect, Scalar(0, 255, 0), 1); // 使用细框表示朝向边缘的装甲板
    }
